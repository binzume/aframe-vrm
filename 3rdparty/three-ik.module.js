...
let { AxesHelper, Color, ConeBufferGeometry, Math: Math$1, Matrix4, Mesh, MeshBasicMaterial, Object3D, Vector3 } = THREE;
...
var IKBallConstraint = function () {
  ...
  createClass(IKBallConstraint, [{
    ...
    value: function _apply(joint) {
      ...
      let z = joint._originalDirection.clone().multiplyScalar(-1);
      var parentDirection = joint._localToWorldDirection(new Vector3().copy(z)).normalize();
  ...
  createClass(IKJoint, [{
    ...
    value: function _applyWorldPosition() {
      ...
      if (parent) {
        ...
        this.bone.quaternion.multiply(new THREE.Quaternion().setFromUnitVectors(new Vector3(0, 0, 1), this._originalDirection));
      } else {
        this.bone.position.copy(position);
      }
